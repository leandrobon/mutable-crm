...
`strict: true` guarantees the arguments match the tool's schema, and that stops
...
Open http://localhost:3000. The database starts empty, so the first thing you do is
ask for some tables.
...
vocabulary rather than disabled behind a check, and that absence *is* the security
boundary.
